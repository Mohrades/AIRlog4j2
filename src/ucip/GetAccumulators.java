...
public class GetAccumulators {
    ...
    public StringBuffer formerRequete(String msisdn, int[][]accumulatorSelection) {
    	...
    	if((accumulatorSelection != null) && (accumulatorSelection.length > 0)) {
	    	int a = accumulatorSelection.length;
	        chaine.append("<member><name>accumulatorSelection</name><value><array><data>");

	        for (int j=0; j < a; j++) {
	        	if((accumulatorSelection[j][0] <= 0) || (accumulatorSelection[j][1] <= 0));
	        	else {
		        	chaine.append("<value><struct><member><name>accumulatorIDFirst</name><value><int>");
